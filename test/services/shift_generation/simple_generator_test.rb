...
class ShiftGeneration::SimpleGeneratorTest < ActiveSupport::TestCase
  ...
  test '平日は休み系と希望休を優先し中勤候補から区なし中勤を入れる' do
    ...
    leave_employee = create_employee(user: user, name: '希望休スタッフ', display_order: 1, assignable_zones: [zone_b],
                                     primary_zone: zone_b, mixed_zone_preferred: true)
    middle_employee = create_employee(user: user, name: '中勤スタッフ', display_order: 2, assignable_zones: [zone_a, zone_b],
                                      primary_zone: zone_a, mixed_zone_preferred: true)
    regular_employee_a = create_employee(user: user, name: '通常スタッフA', display_order: 3, assignable_zones: [zone_a],
                                         primary_zone: zone_a)
    regular_employee_b = create_employee(user: user, name: '通常スタッフB', display_order: 4, assignable_zones: [zone_b, zone_c],
                                         primary_zone: zone_b)
    ...
    regular_assignments = shift_day.shift_assignments.where(employee: [regular_employee_a, regular_employee_b],
                                                            work_type: :day_shift)
  ...
  test '祝日は二人出勤で残り祝日休、出勤者の祝日休は同週内で振り替える' do
    ...
    weekday_rest_days = shift_period.shift_days.where(target_date: Date.new(2026, 4,
                                                                            13)..Date.new(2026, 4, 17)).where.not(id: holiday.id)
    compensatory_assignments = ShiftAssignment.where(shift_day: weekday_rest_days, employee: employees.first(2),
                                                     work_type: :national_holiday)

    assert_equal %w[day_shift middle_shift national_holiday national_holiday].sort,
                 holiday.shift_assignments.pluck(:work_type).sort
