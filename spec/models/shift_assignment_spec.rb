...
RSpec.describe ShiftAssignment, type: :model do
  let(:shift_day) do
    create(:shift_day, period_start_date: Date.new(2026, 4, 15), period_end_date: Date.new(2026, 4, 17),
                       target_date: Date.new(2026, 4, 15))
  end
  ...
  describe 'バリデーション' do
    ...
    it '土日祝の日勤は1人までしか登録できないこと' do
      holiday_shift_day = create(:shift_day, period_start_date: Date.new(2026, 4, 19), period_end_date: Date.new(2026, 4, 21),
                                             target_date: Date.new(2026, 4, 19), day_type: :sunday)
      ...
      assignment = build(:shift_assignment, shift_day: holiday_shift_day, employee: other_employee, zone: nil,
                                            work_type: :day_shift)
  ...
  describe 'zone の制御' do
    ...
    it '祝日の中勤では検証前に zone が外れること' do
      holiday_shift_day = create(:shift_day, period_start_date: Date.new(2026, 4, 20), period_end_date: Date.new(2026, 4, 22),
                                             target_date: Date.new(2026, 4, 20), day_type: :holiday)
      holiday_zone = create(:zone)
      holiday_employee = create(:employee, :with_zone, user: holiday_shift_day.shift_period.user, assignable_zone: holiday_zone)
      assignment = build(:shift_assignment, shift_day: holiday_shift_day, employee: holiday_employee, zone: holiday_zone,
