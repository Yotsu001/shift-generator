module ShiftGeneration
  class SimpleGenerator
    ...
    def assign_mixed_zone_for_day(shift_day, assigned_zone_ids)
      mixed_zone = mixed_zone_record
      return if mixed_zone.blank?
      return if assigned_zone_ids.include?(mixed_zone.id)

      user = mixed_zone_candidate_users_for(shift_day).first
      ...
      if assignment.save!
        assigned_zone_ids << mixed_zone.id
      end
    end
